...
desc "clean all autogenerated files"
task :clean do
  puts "Cleaning build files.".yellow
  FileUtils.rm_rf(Dir.glob(SITE_BUILD_PATH + '/*'))
  puts "Build files deleted.".green

  puts "Cleaning lint files.".yellow
  FileUtils.rm_rf(Dir.glob(CSS_LINT_PATH + '/*'))
  puts "Lint files deleted.".green
end # task :clean

desc "build site"
task :build do
  puts "Building jekyll site.".yellow
  begin
